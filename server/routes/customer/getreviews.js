...
router.post("/", (req, res) => {
  ...
  let command = `SELECT r.*, u.Name AS Name, c.Image, u.ID AS UserID FROM review r INNER JOIN user u ON r.CustomerID = (SELECT ID from customer WHERE UserID = u.ID) AND r.ProductID = ${pid} INNER JOIN customer c ON r.CustomerID = c.ID ORDER BY r.Date DESC`;
